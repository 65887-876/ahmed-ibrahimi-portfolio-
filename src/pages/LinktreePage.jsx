...
export default function LinktreePage() {
  return (
    <section id="contact" className="relative isolate min-h-[calc(100vh-56px)] py-4 sm:py-14">
      <div className="pointer-events-none absolute inset-0 -z-10 bg-linear-to-b from-brand/16 via-transparent to-sea/10" />
      <div className="mx-auto w-full max-w-xl px-3 sm:px-6">
        <div className="rounded-3xl border border-zinc-800/80 bg-zinc-900/55 p-3 shadow-2xl shadow-black/30 backdrop-blur-md sm:p-7">
          <div className="text-center">
            <picture>
              ...
              <img
                ...
                className="mx-auto h-20 w-20 rounded-full border border-zinc-700/80 object-cover shadow-lg shadow-black/30 sm:h-24 sm:w-24"
                loading="eager"
              />
            </picture>
            <p className="mt-3 text-[11px] font-medium uppercase tracking-[0.2em] text-zinc-500 sm:mt-4 sm:text-xs">
              Official linktree
            </p>
            <h1 className="font-display mt-2 text-[2rem] leading-tight text-zinc-50 sm:text-4xl">Ahmed Ibrahimi</h1>
            <p className="mt-1 text-sm text-zinc-400">Photographer</p>
            <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-zinc-400 sm:mt-3">
            ...
            {links.map((item) => {
              ...
              if (item.internal) {
                return (
                  <Link
                    key={item.label}
                    to={item.href}
                    className="group flex items-center justify-between gap-2.5 rounded-2xl border border-brand/45 bg-brand/14 px-3 py-3 transition hover:border-brand-hover hover:bg-brand/22 sm:gap-3 sm:px-4 sm:py-4"
                  >
                    <span className="flex min-w-0 items-center gap-2.5 sm:gap-3">
                      <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-brand/35 bg-zinc-900/85 text-brand-hover sm:h-10 sm:w-10">
                        <Icon className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                      </span>
                      <span className="min-w-0 text-left">
                        <span className="block text-sm font-semibold text-zinc-100">{item.label}</span>
                        <span className="block truncate text-xs text-zinc-400">{item.caption}</span>
                      </span>
                    </span>
                    <span className="shrink-0 text-[11px] uppercase tracking-[0.14em] text-zinc-300">Open</span>
                  </Link>
                )
              }
              ...
              return (
                <a
                  ...
                  className="group flex items-center justify-between gap-2.5 rounded-2xl border border-zinc-700/70 bg-zinc-900/65 px-3 py-3 transition hover:border-sea/45 hover:bg-zinc-900/90 sm:gap-3 sm:px-4 sm:py-4"
                >
                  <span className="flex min-w-0 items-center gap-2.5 sm:gap-3">
                    <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-zinc-700/80 bg-zinc-950/70 text-sea-hover sm:h-10 sm:w-10">
                      {item.label === 'WhatsApp' ? (
                        <svg viewBox="0 0 16 16" aria-hidden="true" className="h-3.5 w-3.5 fill-current sm:h-4 sm:w-4">
                  ...
                  <span className="shrink-0 text-[11px] uppercase tracking-[0.14em] text-zinc-500 transition group-hover:text-zinc-300">
                    Open
                  </span>
                </a>
              )
          ...
          <p className="mt-4 text-center text-[11px] text-zinc-500 sm:mt-5 sm:text-xs">
            © {new Date().getFullYear()} Ahmed Ibrahimi Photography
          </p>
          <p className="mt-1 text-center text-[11px] leading-relaxed text-zinc-400 sm:text-xs">
            Developed by{' '}
            <a
